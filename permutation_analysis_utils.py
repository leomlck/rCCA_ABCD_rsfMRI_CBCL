...
def perm_components_reg(n_permutations, n_components, str_sites, folder_path, training_data, testing_data, cbcl_cols, rsfmri_cols):
    n_cbcl = len(cbcl_cols)
    n_con = len(rsfmri_cols)
    cors = np.zeros((2, len(str_sites), n_components, n_permutations))
    loadings_cbcl = np.zeros((2, len(str_sites), n_components, n_cbcl, n_permutations))
    ...
    for i_s, str_site in enumerate(str_sites):
        ...
        best_cca = CCA(latent_dimensions=n_components, **best_params)
        for i_p in range(n_permutations):
            train_cbcl_p = training_data[i_s][cbcl_cols].sample(frac=1)
            test_cbcl_p = testing_data[i_s][cbcl_cols].sample(frac=1)
            X1_c_train, X2_c_train = best_cca.fit_transform((training_data[i_s][rsfmri_cols], train_cbcl_p))
...
def perm_components_sex2(n_permutations, n_components, str_sites, folder_path, training_data, testing_data, cbcl_cols, rsfmri_cols):
    sexes = {'male':1, 'female':2}
    n_cbcl = len(cbcl_cols)
    n_con = len(rsfmri_cols)
    cors = np.zeros((2, len(str_sites), n_components, n_permutations))
    ...
    for i_s, str_site in enumerate(str_sites):
        ...
        male_data_train = training_data[i_s][training_data[i_s]['demo_sex_v2']==sexes['male']]
        male_data_test = testing_data[i_s][testing_data[i_s]['demo_sex_v2']==sexes['male']] 
        female_data_train = training_data[i_s][training_data[i_s]['demo_sex_v2']==sexes['female']] 
        female_data_test = testing_data[i_s][testing_data[i_s]['demo_sex_v2']==sexes['female']]

        best_cca = rCCA(latent_dimensions=n_components, **best_params)
        for i_p in range(n_permutations):
            best_cca.fit((male_data_train[rsfmri_cols], male_data_train[cbcl_cols].sample(frac=1)))
            X1_c_male, X2_c_male = best_cca.transform((male_data_test[rsfmri_cols], male_data_test[cbcl_cols].sample(frac=1)))
            best_cca.fit((female_data_train[rsfmri_cols], female_data_train[cbcl_cols].sample(frac=1)))
            X1_c_female, X2_c_female = best_cca.transform((female_data_test[rsfmri_cols], female_data_test[cbcl_cols].sample(frac=1)))
